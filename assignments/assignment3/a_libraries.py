...
def matplotlib_line_example2():
    """
    Now we can call the other method and add more parameters to the axis if we want before showing it
    """
...
def matplotlib_table(matrix: np.array) -> Tuple:
    ...
    """
    fig, ax = plt.subplots()
    ax.table(cellText=matrix, colLabels=list(range(matrix.shape[1])))
    return fig, ax
...
def plotly_polar_chart(df: pd.DataFrame):
    ...
    """
    df2 = df.copy()
    df2['angle'] = 2 * np.pi * df2['y']
    fig = px.line_polar(df2, r="x", theta="angle")
    return fig
...
def plotly_table(df: pd.DataFrame):
    ...
    Return the fig only. Feel free to choose between px and go.
    """
    tableData = [df[x] for x in list(df.columns)]
    # print(tableData)
    tableData.insert(0, df.index.to_list())
...
if __name__ == "__main__":
    ...
    # Setting the see as pie chart won't accept negative values
    np.random.seed(4)
