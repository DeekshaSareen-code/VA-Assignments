...
def process_iris_dataset_again() -> pd.DataFrame:
    ...
    df_petal_width_mean = df["petal_width"][(df["petal_width"] >= 0) & (df["petal_width"] <= 1)].mean()
    df.loc[(df["petal_width"] < 0) & (df["petal_width"] > 1), 'petal_width'] = df_petal_width_mean
...
def process_life_expectancy_dataset():
    ...
    # ASSUMPITION 'four_regions' is the continent attribute
    ...
    le = generate_label_encoder(total.loc[:, "Latitude"])
    total = replace_with_label_encoder(total, 'Latitude', le)
...
if __name__ == "__main__":
    ...
    # print(process_iris_dataset())
    # print(process_iris_dataset_again())
    # print(process_amazon_video_game_dataset())
    # print(process_amazon_video_game_dataset_again())
    print(process_life_expectancy_dataset())