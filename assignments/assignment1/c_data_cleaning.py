...
def standardize_column(df_column: pd.Series) -> pd.Series:
    ...
    df_column = df_column.apply(lambda x: 2*((x - min_val)/(max_val-min_val)) - 1)
    return df_column
...
if __name__ == "__main__":
    ...
    # print(fix_numeric_wrong_values(df, 'a', WrongValueNumericRule.MUST_BE_LESS_THAN, 4))
    # print(fix_numeric_wrong_values(df, 'a', WrongValueNumericRule.MUST_BE_GREATER_THAN, 2))
    # print(fix_numeric_wrong_values(df, 'a', WrongValueNumericRule.MUST_BE_POSITIVE, 2))
    # print(fix_numeric_wrong_values(df, 'a', WrongValueNumericRule.MUST_BE_NEGATIVE, 2))
    # print(fix_outliers(df, 'c'))
    # print(fix_nans(df, 'c'))
    # print(normalize_column(df.loc[:, 'a']))
    print(standardize_column(df.loc[:, 'a']))
    # print(calculate_numeric_distance(df.loc[:, 'a'], df.loc[:, 'a'], DistanceMetric.EUCLIDEAN))
    # print(calculate_numeric_distance(df.loc[:, 'a'], df.loc[:, 'a'], DistanceMetric.MANHATTAN))
    # print(calculate_binary_distance(df.loc[:, 'b'], df.loc[:, 'b']))
    print("ok")